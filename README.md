...
- ✅ **资金管理**：支持资金分配和额度管理
- ✅ **期权日内策略（买方）**：新增 `OPTION_INTRADAY_V1`，自动选合约 + 费用模型 + 收盘前强平

**新增策略类型**：
- `RECOMMENDATION_V1`（股票推荐策略）
- `OPTION_INTRADAY_V1`（期权日内策略，买方）

**期权日内策略（OPTION_INTRADAY_V1）要点**：
- **标的池**：输入 underlying（如 `QQQ.US`、`TSLA.US`、`.SPX.US`）
- **合约选择**：0DTE/最近到期 + ATM附近 strike + 流动性（OI/价差）与 Greek（delta/theta）过滤
- **硬约束**：
  - 收盘前 **30 分钟强制平仓**（不论盈亏）
  - 收盘前 N 分钟 **禁止开新仓**（默认 60，可配置）
- **费用模型**：按张计费并计入资金占用（佣金最小 0.99 + 平台费每张 0.30）
...
- 节流机制：30秒窗口去重，同类消息仅入库一次
- 摘要聚合：新增log-digest.service.ts，每5分钟聚合高频指标
- 基础设施Logger：新增infra-logger.ts，解决底层模块循环依赖
- console全量迁移：约38个文件、398处console.*迁移到logger.*
- 33个单元测试全部通过
...
**详细内容**: 参考 [日志系统重构](docs/features/260206-日志系统重构.md)

---

### 2026-02-06: 期权策略风控优化 ⭐ 关键优化

**优化**: 优化期权止盈止损执行方式，增强订单追踪能力，提升期权策略风控可靠性

**核心改进**:
- 期权止盈止损改用市价单(MO)，避免限价单无法成交导致亏损扩大
- 修复SELL订单信号关联问题，新增logSellSignal方法确保订单可追踪
- 防止已完成订单重复尝试信号匹配，减少日志噪音
- 新增option-dynamic-exit.service实现动态止盈止损
- 增强期权合约选择和价格缓存服务
- 前端EditStrategyModal支持更多期权策略配置
...
**详细内容**: 参考 [CHANGELOG.md](CHANGELOG.md)
...
**优化**: 为期权日内策略增加9个关键检查点的详细日志，并提供快捷分析工具

**核心改进**:
- 9个关键检查点覆盖完整决策链路（市场数据 → 信号判定 → 期权选择 → 信号生成）
- 统一日志格式（`📍 [标的符号] 描述 | 指标=值`），方便快速定位
- 快捷分析工具（`analyze-latest.bat`一键分析最新日志）
- 自动生成可视化报告（HTML看板 + 文本报告）

**使用方法**:
1. 从系统前端导出今日日志（JSON格式）
2. 双击运行 `analyze-latest.bat`
3. 自动打开HTML看板查看分析结果

**详细内容**: 参考 [期权策略决策链路日志增强](docs/features/260204-期权策略决策链路日志增强.md)
...
**更新内容**：
1. ✅ 卖空验证失败写入 `validation_failure_logs`（便于追踪与分析）
2. ✅ 卖空下单与订单改价参数修复（`submittedQuantity` 传正数、`replaceOrder.quantity` 用 `Decimal`）
3. ✅ LongPort 关键调用增加限流 + 429002 指数退避重试
