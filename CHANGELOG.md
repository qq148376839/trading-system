...
**方案**: Docker Compose Override 模式，同一份代码通过 `docker-compose.test.yml` 覆盖端口(3003)、容器名、volume、network 实现完全隔离。前端编译时注入 `NEXT_PUBLIC_ENV_MODE=test`，AppLayout 顶部显示橙色 "TEST ENVIRONMENT" 横幅（不可关闭）。
...
**trading-session.service.ts**: `isInTradingSession()` 前置 `isTradingDay()` 检查 — LongPort `tradingSession()` 假日仍返回标准时段，必须先拦截非交易日
...
**关键修正**:
- **C1**: TIME_STOP 行号 L530-540 → L509-521（实际代码位置）
- **C2**: 区分天级方向准确率(36%)与交易级胜率(44%)，修正盈亏平衡论证(WR 距平衡仅差 3pp)
- **C3**: 统一 Phase 编号 — Phase 1(退出修复)/2(A+D)/3(方向信号)/4(动态参数)
- **Math.round**: 阈值计算补充 `Math.round()`（L298），VIX=10 时最低阈值=5 非 4.5
- **M2**: BTC 噪音双路径(intradayScore+marketScore)，合计影响 11-13% 非 9%
- **M5**: TIME_STOP "优先级错误"→"覆盖间隙"精确表述

**修改文件**: 3 份分析文档（纯文档，无代码变更）
...
**背景**: 3/24 SPY P655 入场方向正确（PUT），但 8 分钟后被 0DTE 硬编码兜底止损 -25% 拦截（实际亏 -26%），而策略配置止损为 -30%。后续 SPY 继续走弱，该笔若存活大概率盈利。同日 QQQ P585 也因同一兜底被"误杀"（-26.7%）。

**改动**: `option-dynamic-exit.service.ts` L510: `zdtePnlFloor` 从硬编码 `25` 改为 `dynamicParams.stopLossPercent`，与策略配置一致。安全阀（L558 硬上限 40%）仍保留。

**修改文件**: `api/src/services/option-dynamic-exit.service.ts`
...
**问题**: 每日重置逻辑只扫描 `NOT IN ('IDLE')` 的实例，IDLE 状态标的（如无交易日的 QQQ/SPY）的 `dailyRealizedPnL` 永远不会清零。前几天的亏损持续累积，导致熔断阈值错误触发（显示 $2470 累计亏损，实际今天仅亏 $48）。

**修复**:
1. 主重置去掉 IDLE 过滤，扫描策略下所有实例
2. 无时间戳但有残留 PnL 的实例也触发重置（兜底）
3. 入场时两处重置检查点同步加固
...
1. **期权K线采集服务**: 收盘后自动采集正向+反向期权K线，防止期权过期后数据丢失
2. **交易分析页面 `/quant/analysis`**: 4个Tab — 综合分析（策略/退出/盈亏图表/得分散点）、每日明细、反向分析（原始vs反向对比）、K线图表
...
- `frontend/components/AppLayout.tsx`（导航+面包屑）
- `frontend/lib/api.ts`（4个新API方法）

---

### 智能反向策略 (Smart Regime Reversal)

**改动内容**:

1. **新建 MarketRegimeDetector 服务**: 独立市场状态判别层，基于分数极端度 + 分量分歧度 + 时间窗口判定 MOMENTUM / MEAN_REVERSION / UNCERTAIN 三种状态
2. **替代 REVERSE_BEAR/BULL_SPREAD**: 旧的"无脑反向"策略被智能判别替代，旧配置自动兼容（跳过 REVERSE_* 不报错）
3. **前端智能反向配置面板**: StrategyFormModal 新增开关 + 阈值配置（大盘极端/日内极端/分歧度/截止时间/UNCERTAIN仓位系数）
4. **信号日志 regime 标签**: 信号页面显示"反向"/"不确定"标签

**设计文档**: `docs/features/260314-智能反向策略设计方案.md`

**修改文件**:
- `api/src/services/market-regime-detector.service.ts`（新建）
- `api/src/services/strategies/option-intraday-strategy.ts`（集成 regime detection）
- `frontend/components/StrategyFormModal.tsx`（智能反向配置面板）
- `frontend/app/quant/signals/page.tsx`（regime 标签）
...
1. **盈利退出**: 冷却 10 分钟（原 3 分钟），防止追高重入
2. **亏损退出**: 冷却 20 分钟（原 3 分钟），给市场充分反转时间
3. **连亏 ≥2 次**: 当日禁止该标的交易（新增 `NON0DTE_DAILY_BAN` 日志标签）
4. **0DTE 冷却逻辑不变**（已有完善升级机制）

**修改文件**:
- `api/src/services/strategy-scheduler.service.ts`（`evaluateIdleSymbol` 非0DTE冷却分支）
- `docs/analysis/260313-组内竞价轮换与冷却升级分析.md`（分析报告）
...
1. **温度历史表**: 新建 `market_temperature_history` 表，存储实盘 Longport API 返回的真实温度（每 5 分钟一条，5 分钟窗口去重）
2. **实盘写温度**: `getMarketTemperature()` 成功获取温度后异步写入 DB（不阻塞主流程）
3. **回测读真实温度**: `buildMarketDataWindow()` 优先从 DB 读取真实温度（±5 分钟窗口），无数据时回退到 `estimateMarketTemperature()`
4. **日K close 分时修正**: `calculateMarketScore()` 入口处用分时/时K最新价修正 SPX/USD/BTC 日K 当天最后一根 bar 的 close，消除日K缓存延迟
...
1. **交易窗口对齐实盘 `firstHourOnly`**: `resolveStrategyConfig` 从 `tradeWindow.firstHourOnly` 推导 `tradeWindowEndET`（`true`→630, `false`→`960 - noNewEntryBeforeCloseMinutes`），不再读不存在的 `tradeWindowStartET/EndET` 字段
2. **评分曲线平滑**: `calculateTimeWindowAdjustment` 从阶梯函数改为渐变曲线 — 9:30-10:00 +15, 渐退至10:30 +5, 平滑到11:30 0, 午间0, 午后渐降-15, 收盘前-50。10:30不再断崖（旧: +20→0 跌4分, 新: +5→0 跌1分）
3. **前端日期改 RangePicker**: 替换 `DatePicker multiple` 为 `RangePicker`（含快捷预设），选区间后调用 `/api/trading-days/get-trading-days` 获取精确交易日，自动排除非交易日（总统日等）
4. **日期展示优化**: 列表从逗号分隔改为 `2026-02-19 ~ 2026-02-25 (5天)` 格式
5. **策略窗口自动匹配**: 选策略后根据 `tradeWindow.firstHourOnly` 自动设置交易窗口下拉框

**修改文件**:
- 📝 `api/src/services/option-backtest.service.ts`（`resolveStrategyConfig` + `calculateTimeWindowAdjustment`）
- 📝 `frontend/app/quant/backtest/page.tsx`（RangePicker + 交易日API + 日期展示 + 策略窗口联动）
- 📝 `frontend/lib/api.ts`（新增 `tradingDaysApi.getTradingDays`）

---

### 期权回测数据分析 — 交易窗口 + 评分曲线问题定位（P2 待优化）

**背景**: 对回测 #86（8天/20笔交易）进行深度分析，发现回测引擎与实盘策略在交易窗口和评分曲线上存在显著差异。

**分析结果**:

1. **交易窗口未对齐实盘**: `resolveStrategyConfig` 读 `stratConfig?.tradeWindowStartET/EndET`（策略 DB 无此字段），永远 fallback 630（10:30 ET）。实盘 `option-intraday-strategy.ts:306` 用 `tradeWindow.firstHourOnly` 布尔值，`false` 时全天可交易
2. **timeWindowAdjustment 断崖**: `calculateTimeWindowAdjustment()` 在10:30从 +20 直降到 0（加权 -4分），中等强度信号在10:30后全被过滤
3. **marketScore 接近零**: `marketTemperature` 回测中固定50（贡献恒为0），SPX趋势偏离<1%（贡献约1分），大盘20%权重形同虚设
4. **前端日期含非交易日**: `DatePicker multiple` 无交易日过滤，用户误选总统日等假日
5. **120%+ 止盈属正常**: 0DTE高gamma + 1min分辨率，TAKE_PROFIT在candle close检测时已超过50%目标（对交易者有利）
6. **阈值已确认对齐**: 实盘和回测都用 `directionalScoreMin × vixFactor`

**修复计划**: 已记录为 P2 待优化项（见 PROJECT_STATUS.md），包含 3 个子项：窗口对齐、曲线平滑、日期选择器
