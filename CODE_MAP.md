...
**最后更新**: 2026-02-06（日志系统全面重构：级别门控、节流机制、摘要聚合、infra-logger、console全量迁移）
...
- ✅ 使用 `utils/longport-rate-limiter.ts` - LongPort 调用限流 + 429002 重试（改单等关键调用）
...
**作用**: 市场数据获取服务（富途 API + LongPort SDK）

**主要功能**:
- 获取 K线数据（SPX、USD Index、BTC、VIX）
- 获取分时数据
...
**主要方法**:
- `getMarketTemperature()` - 获取当前市场温度（0-100范围）
...
**主要功能**:
- 综合分析市场数据（SPX、USD、BTC、VIX、市场温度）
- 计算技术指标（ATR、Z-score）
- 生成交易信号（BUY/SELL/HOLD）
- 计算入场价、止损价、止盈价
- 市场状态矩阵计算（Goldilocks、Volatile Bull、Fear、Stagnant）
- 一票否决权机制（VIX > 35 或 温度 < 10）
- 环境分计算（基础强度40% + 温度40% + VIX20%）
...
**主要方法**:
- `calculateRecommendation()` - 计算单个股票的交易推荐
- `getMarketRegime()` - 获取市场状态矩阵（全局市场环境指标）
- `calculateTradingDecision()` - 综合计算交易决策（集成市场状态矩阵）
...
**主要方法**:
- `getOptionDetail()` - 获取期权详情（新增 `underlyingPrice` 等便捷字段）
- `getOptionStrikeDates()` - 获取期权到期日期列表
- `getOptionChain()` - 获取期权链数据
...
- 缓存期权实时价格（5分钟TTL）
- 自动清理过期缓存（每10分钟）
- 记录价格来源（longport/futunn/position_cache）
- 提供统计信息查询
...
**被调用**:
- 📌 `services/strategy-scheduler.service.ts` - 期权持仓监控（价格获取）

**主要方法**:
- `set(symbol, entry)` - 设置缓存
- `get(symbol)` - 获取缓存（自动检查过期）
- `getStats()` - 获取缓存统计信息
- `startCleanup()` - 启动自动清理（应用启动时自动调用）
...
- ✅ 使用 `services/strategies/recommendation-strategy.ts` - 推荐策略
- ✅ 使用 `services/strategies/option-intraday-strategy.ts` - 期权日内策略（买方）
- ✅ 使用 `services/stock-selector.service.ts` - 选股器
...
**主要功能**:
- 基于 underlying 的推荐信号决定方向（BUY→Call，SELL→Put，可配置）
...
**作用**: 统一日志工具（级别门控 + 节流 + 摘要聚合入口）

**主要功能**:
- 提供统一的日志接口（log、info、warn、error、debug）
- 级别门控：DEBUG仅控制台、ERROR必入库不节流、WARN入库走节流、INFO可选跳过
- 支持 `{dbWrite:false}` 选项跳过数据库写入
- `console()` 方法：仅输出到控制台，不入库
- `metric()` 方法：标记为指标类日志，参与摘要聚合
- 自动添加时间戳

**调用关系**:
- ✅ 使用 `services/log.service.ts` - 入队列写库
- ✅ 控制台输出始终执行

**被调用**:
- 📌 所有业务服务文件（约38个文件，约398处调用）

#### `api/src/utils/infra-logger.ts`
**作用**: 基础设施轻量 Logger（避免循环依赖）

**主要功能**:
- 仅依赖 `console.*`，无外部依赖
- 专供底层基础设施模块使用
- 解决 logger -> log.service -> config.service -> logger 的循环依赖
...
- 📌 `services/log-worker.service.ts` - 日志工作线程
...
**作用**: LongPort API 调用保护（限流 + 429002 指数退避重试）

**主要功能**:
- 轻量队列限流（最小间隔）
- 识别 LongPort 429002 限流错误并指数退避重试

**被调用**:
- 📌 `services/basic-execution.service.ts` - 下单 submitOrder
- 📌 `services/strategy-scheduler.service.ts` - 改单 replaceOrder（订单追踪）
- 📌 `routes/orders.ts` - 改单 replaceOrder（API路由）
- 📌 `services/short-position-validation.service.ts` - accountBalance（卖空权限/保证金校验）
...
4. **日志记录**: 所有业务服务使用 `utils/logger.ts` 进行日志记录；底层基础设施模块（config、database、log-worker、log.service）使用 `utils/infra-logger.ts` 避免循环依赖
...
**新增文件**:
- `api/src/utils/infra-logger.ts` - 基础设施轻量Logger（避免循环依赖）
  - 调用关系：仅使用 console.*，无外部依赖
  - 被调用：config.service.ts、log-worker.service.ts、database.ts、log.service.ts
- `api/src/services/log-digest.service.ts` - 摘要聚合服务
  - 调用关系：使用 config.service.ts（读取配置）、log.service.ts（写入聚合日志）
  - 被调用：server.ts（初始化和关闭）
- `api/migrations/013_add_log_throttle_digest_config.sql` - 节流/摘要配置项迁移脚本

**修改文件**:
- `api/src/utils/logger.ts` - 新增级别门控、{dbWrite:false}选项、console()/metric()方法
  - 变更内容：DEBUG不入库、ERROR不节流、WARN/INFO走节流门控
- `api/src/services/log.service.ts` - 新增节流机制
  - 变更内容：添加throttleMap、generateThrottleKey()、shouldEnqueue()
- `api/src/utils/log-module-mapper.ts` - 新增Log.Digest映射
- `api/src/server.ts` - 新增digest服务初始化和关闭
- 约38个业务文件 - console.*迁移到logger.*（约398处）

**基础设施迁移**（console.* -> infraLogger.*）:
- `api/src/services/config.service.ts`
...
**调用关系变更**:
- server.ts 新增依赖：log-digest.service.ts（初始化/关闭）
- logger.ts 强化与 log.service.ts 的交互（级别门控逻辑）
- 4个基础设施文件从 console.* 改为 infra-logger.ts
...
### 期权策略完整修复 ✅
- ✅ **资金管理修复**：
  - 资金释放逻辑优先使用 `allocationAmount`（包含完整成本：premium×multiplier×contracts+fees）
  - 持仓状态保存统一使用 `allocationAmountOverride`
  - 从历史订单恢复完整成本
  - 详细的fallback日志和警告
- ✅ **期权价格缓存服务**：
  - 新增 `option-price-cache.service.ts`
  - 5分钟TTL，减少API调用60%
  - 记录价格来源（longport/futunn/position_cache）
- ✅ **期权价格获取增强**：
  - 三层降级机制（LongPort → 持仓缓存 → 富途详情）
  - 缓存优先检查
  - 详细的失败日志
- ✅ **期权合约选择优化**：
  - 修复0DTE到期日期验证（检查是否仍在交易）
  - 修复Greeks数据缺失处理（区分"不可用"和"值为0"）
  - 补充指数stockId映射（SPX/SPXW/XSP系列）
  - 详细的过滤日志（记录Delta、Theta、价差过滤原因）
- ✅ **期权详情接口增强**：
  - 添加顶层便捷字段（underlyingPrice、underlyingChange等）
  - 避免嵌套访问
- ✅ **边缘函数优化**：
  - 明确可选参数处理（lotSize）
  - 清晰的注释说明
...
- ✅ **单元测试**：9/9通过（100%）
  ...
  - 市场时段计算测试（2个用例）
- ✅ **API集成测试**：3/4通过（75%）
  - 期权合约选择测试（1个因过滤条件严格未通过，属正常）
  - 期权详情接口测试（5个用例全部通过）
- ✅ **测试文档**：`docs/test/260128-期权策略修复验证报告.md`
...
### LongPort SDK 升级和修复 ✅
- ✅ **SDK版本升级**：从1.1.7升级到3.0.18
- ✅ **市场温度功能**：成功实现市场温度获取（值：70.0）
- ✅ **API调用修复**：修复所有`candlesticks`方法调用，添加`TradeSessions`参数
  - `trading-recommendation.service.ts` - `getStockCandlesticks`方法
  - `market-data.service.ts` - `getVIXCandlesticks`方法
  - `backtest.service.ts` - `getHistoricalCandlesticks`方法（2处修复）
- ✅ **测试体系建设**：创建市场状态矩阵测试（21个测试用例，100%通过）

### 市场状态矩阵测试完成 ✅
- ✅ **测试文件**：`api/src/tests/market-regime-matrix.test.ts`（527行）
- ✅ **测试覆盖**：
  - 市场状态矩阵计算（4种状态）
  - 一票否决权机制
  - 环境分计算
  ...
  - 完整策略影响
  - 边界条件处理
- ✅ **测试文档**：`api/src/tests/MARKET_REGIME_MATRIX_TEST.md`
...
### 期权策略决策链路日志增强（2026-02-04）
- ✅ **9个关键检查点日志**：
  - 检查点1-3：`option-recommendation.service.ts`（市场数据、信号判定、风险评估）
  - 检查点4-7：`options-contract-selector.service.ts`（期权日期、期权链、流动性、Greeks筛选）
  - 检查点8-9：`option-intraday-strategy.ts`（入场价格、信号生成）
- ✅ **快捷分析工具**：
  - `analyze-today.bat`（分析指定日志文件）
  - `analyze-latest.bat`（自动分析最新日志）
- ✅ **统一日志格式**：`📍 [标的符号] 描述 | 指标=值`
