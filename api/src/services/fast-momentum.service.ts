...
const ACCEL_MIN_SLOPE = 0.0005;    // slope 绝对值门槛，低于此视为无方向
...
class FastMomentumService {
  ...
  /**
   * 获取加速度信息 — 用于 Phase 4 动量加速度 bonus
   * 使用短 buffer（60s 窗口）计算前后半段 slope 比值
   * @param symbol 标的代码
   * @param accelRatioThreshold decelRatio 超过此值才视为加速（默认 1.15）
   */
  getAccelerationInfo(symbol: string, accelRatioThreshold: number = 1.15): AccelerationInfo {
    const buf = this.buffers.get(symbol);
    const noData: AccelerationInfo = {
      isAccelerating: false, magnitude: 0, direction: null,
      slope: null, decelRatio: null, reliable: false, dataPoints: buf?.size() ?? 0,
    };
    ...
    if (!sameDirection) {
      return {
        isAccelerating: false, magnitude: 0, direction,
    ...
    return {
      isAccelerating, magnitude, direction,
      slope: fullReg.slope, decelRatio, reliable: true, dataPoints: n,
    };
  }
...
// 导出类和辅助函数供测试使用
export { FastMomentumService, RingBuffer, linearRegression, BUFFER_CAPACITY, MIN_DATA_POINTS, DECEL_THRESHOLD, DECEL_UPPER_BOUND, NEAR_ZERO_SLOPE, LONG_BUFFER_CAPACITY, LONG_MIN_DATA_POINTS, ACCEL_MIN_DATA_POINTS, ACCEL_MIN_SLOPE };
