# 错误位置定位及修复指南

基于日志分析结果（2026-01-27），本文档定位了所有关键错误在代码中的具体位置，为后续修复提供精确指导。

---

## 📊 错误统计概览

- **总错误数**: 3,737 (15.2%)
- **总警告数**: 4,479 (18.2%)
- **总日志数**: 24,630

---

## 🔴 高优先级错误

### 1. 数据库表缺失错误 (780次)

#### 错误信息
```
[策略执行] 记录验证失败日志失败:
错误码: 42P01 (relation does not exist)
```

#### 代码位置
**文件**: `api/src/services/strategy-scheduler.service.ts`
**行号**: 1118

#### 代码片段
```typescript:api/src/services/strategy-scheduler.service.ts
// 第1100-1120行
try {
  await pool.query(
    `INSERT INTO validation_failure_logs (
      strategy_id,
      symbol,
      failure_type,
      reason,
      timestamp
    ) VALUES ($1, $2, $3, $4, $5)`,
    [
      strategyId,
      symbol,
      'SHORT_VALIDATION_FAILED',
      error.message,
      new Date().toISOString(),
    ]
  );
} catch (dbError: unknown) {
  logger.error(`[策略执行] 记录验证失败日志失败:`, dbError);  // ← 第1118行
  // 不阻塞主流程，只记录错误
}
```

#### 问题分析
- **根本原因**: 数据库表 `validation_failure_logs` 不存在
- **影响范围**: 所有卖空验证失败的情况都无法记录到数据库
- **触发场景**:
  - 卖空权限检查失败时
  - 保证金计算失败时
  - 其他卖空验证失败时

#### 修复方案

##### 方案1: 使用迁移脚本创建表（推荐）

**迁移文件已创建**: `api/migrations/011_create_validation_failure_logs.sql`

执行迁移：

```bash
# Windows PowerShell
$env:PGPASSWORD="your_password"
psql -U postgres -d trading_db -f api\migrations\011_create_validation_failure_logs.sql

# Linux/Mac
PGPASSWORD=your_password psql -U postgres -d trading_db -f api/migrations/011_create_validation_failure_logs.sql
```

或者直接运行SQL：

```sql
-- 创建验证失败日志表（详见 api/migrations/011_create_validation_failure_logs.sql）
BEGIN;

CREATE TABLE IF NOT EXISTS validation_failure_logs (
  id SERIAL PRIMARY KEY,
  strategy_id INTEGER NOT NULL,
  symbol VARCHAR(20) NOT NULL,
  failure_type VARCHAR(50) NOT NULL,
  reason TEXT,
  timestamp TIMESTAMP NOT NULL,
  created_at TIMESTAMP DEFAULT NOW(),
  CONSTRAINT fk_validation_logs_strategy
    FOREIGN KEY (strategy_id)
    REFERENCES strategies(id)
    ON DELETE CASCADE
);

-- 创建索引
CREATE INDEX IF NOT EXISTS idx_validation_logs_strategy_id ON validation_failure_logs(strategy_id);
CREATE INDEX IF NOT EXISTS idx_validation_logs_symbol ON validation_failure_logs(symbol);
CREATE INDEX IF NOT EXISTS idx_validation_logs_timestamp ON validation_failure_logs(timestamp DESC);
CREATE INDEX IF NOT EXISTS idx_validation_logs_failure_type ON validation_failure_logs(failure_type);
CREATE INDEX IF NOT EXISTS idx_validation_logs_strategy_symbol ON validation_failure_logs(strategy_id, symbol, timestamp DESC);

-- 添加表注释
COMMENT ON TABLE validation_failure_logs IS '卖空验证失败日志记录表 - 记录策略执行过程中卖空验证失败的情况';

COMMIT;
```

**验证表创建**：

```sql
-- 检查表是否存在
SELECT table_name FROM information_schema.tables WHERE table_name = 'validation_failure_logs';

-- 查看表结构
\d validation_failure_logs

-- 查看索引
\di validation_failure_logs*

-- 测试插入
INSERT INTO validation_failure_logs (strategy_id, symbol, failure_type, reason, timestamp)
VALUES (1, 'TEST.US', 'TEST', 'Test record', NOW());

-- 清理测试数据
DELETE FROM validation_failure_logs WHERE failure_type = 'TEST';
```

##### 方案2: 移除数据库记录逻辑（临时方案）
如果暂时不需要持久化这些日志，可以注释掉数据库插入代码，只保留日志记录。

```typescript
// 注释掉数据库插入
// try {
//   await pool.query(...);
// } catch (dbError: unknown) {
//   logger.error(`[策略执行] 记录验证失败日志失败:`, dbError);
// }
```

#### 建议
**强烈推荐方案1**，因为验证失败日志对于排查问题和优化策略非常重要。

---

### 2. API请求限流错误 (1,088次)

#### 错误信息
```
openapi error: code=429002: api request is limited, please slow down request frequency
```

#### 代码位置

##### 2.1 策略调度器中的API调用
**文件**: `api/src/services/strategy-scheduler.service.ts`
**涉及行**: 多处API调用

主要触发点：
1. **订单价格更新** (约第800-860行)
2. **持仓查询** (约第400-500行)
3. **卖空权限检查** (通过 shortValidationService)

##### 2.2 卖空验证服务中的API调用
**文件**: `api/src/services/short-position-validation.service.ts`

涉及的API调用：
- `tradeClient.marginRatio()` - 保证金比例查询
- `tradeClient.stockPositions()` - 持仓查询
- 其他交易相关API

#### 问题分析
- **根本原因**: 请求频率超过LongPort API限制（429错误）
- **影响模块**:
  - Strategy.Scheduler: 864次
  - Service.Short.Position.Validation.Service: 224次
- **触发场景**:
  - 策略定时执行时批量查询多个标的
  - 订单监控频繁更新价格
  - 卖空验证时并发查询多个标的的保证金

#### 修复方案

##### 方案1: 实现请求速率限制器（推荐）

创建新文件 `api/src/utils/rate-limiter.ts`:

```typescript
/**
 * API请求速率限制器
 * 防止超过API限流阈值
 */
export class RateLimiter {
  private queue: Array<{ fn: () => Promise<any>; resolve: Function; reject: Function }> = [];
  private processing = false;
  private lastRequestTime = 0;
  private minInterval: number; // 最小请求间隔（毫秒）
  private requestCount = 0;
  private windowStart = Date.now();
  private readonly windowSize = 60000; // 1分钟窗口
  private readonly maxRequestsPerWindow = 100; // 每分钟最多100个请求

  constructor(minInterval: number = 100) {
    this.minInterval = minInterval;
  }

  /**
   * 执行被限流的函数
   */
  async execute<T>(fn: () => Promise<T>): Promise<T> {
    return new Promise((resolve, reject) => {
      this.queue.push({ fn, resolve, reject });

      if (!this.processing) {
        this.processQueue();
      }
    });
  }

  /**
   * 处理队列
   */
  private async processQueue() {
    if (this.processing) return;
    this.processing = true;

    while (this.queue.length > 0) {
      // 检查窗口是否需要重置
      const now = Date.now();
      if (now - this.windowStart >= this.windowSize) {
        this.requestCount = 0;
        this.windowStart = now;
      }

      // 如果达到窗口限制，等待到下一个窗口
      if (this.requestCount >= this.maxRequestsPerWindow) {
        const waitTime = this.windowSize - (now - this.windowStart);
        await new Promise(r => setTimeout(r, waitTime));
        this.requestCount = 0;
        this.windowStart = Date.now();
      }

      // 确保最小间隔
      const elapsed = Date.now() - this.lastRequestTime;
      if (elapsed < this.minInterval) {
        await new Promise(r => setTimeout(r, this.minInterval - elapsed));
      }

      const task = this.queue.shift();
      if (task) {
        try {
          const result = await task.fn();
          this.lastRequestTime = Date.now();
          this.requestCount++;
          task.resolve(result);
        } catch (error) {
          task.reject(error);
        }
      }
    }

    this.processing = false;
  }

  /**
   * 获取当前队列长度
   */
  getQueueLength(): number {
    return this.queue.length;
  }
}

// 创建全局实例
export const apiRateLimiter = new RateLimiter(150); // 每个请求最少间隔150ms
```

##### 方案2: 添加指数退避重试机制

在 `api/src/utils/api-retry.ts` 中实现：

```typescript
/**
 * 带指数退避的API重试
 */
export async function retryWithBackoff<T>(
  fn: () => Promise<T>,
  maxRetries: number = 3,
  initialDelay: number = 1000
): Promise<T> {
  let lastError: any;

  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      return await fn();
    } catch (error: any) {
      lastError = error;

      // 如果是429错误，使用指数退避
      if (error.message?.includes('429002') || error.message?.includes('api request is limited')) {
        if (attempt < maxRetries) {
          const delay = initialDelay * Math.pow(2, attempt);
          console.log(`API限流，等待 ${delay}ms 后重试 (尝试 ${attempt + 1}/${maxRetries})`);
          await new Promise(resolve => setTimeout(resolve, delay));
          continue;
        }
      }

      // 其他错误直接抛出
      throw error;
    }
  }

  throw lastError;
}
```

##### 方案3: 使用位置

在 `strategy-scheduler.service.ts` 中应用：

```typescript
import { apiRateLimiter } from '../utils/rate-limiter';
import { retryWithBackoff } from '../utils/api-retry';

// 原代码：
// const order = await tradeClient.replaceOrder(...)

// 修改后：
const order = await apiRateLimiter.execute(() =>
  retryWithBackoff(() => tradeClient.replaceOrder(...))
);
```

#### 建议
**同时实施方案1和方案2**，既限制并发请求速率，又提供重试机制。

---

### 3. 订单提交失败 - Decimal类型错误 (1,308次)

#### 错误信息
```
策略 5 提交订单失败 (RBLX.US/PLTR.US/ROKU.US等):
openapi error: code=602001: invalid SubmitRequest.SubmittedQuantity:
value does not match regex pattern "^([1-9]\\d*(\\.\\d+)?)$"
```

#### 代码位置
**文件**: `api/src/services/basic-execution.service.ts`
**行号**: 778

#### 代码片段
```typescript:api/src/services/basic-execution.service.ts
// 第690-778行
async submitOrder(
  strategyId: number,
  symbol: string,
  intent: TradingIntent
): Promise<ExecutionResult> {
  try {
    // ... 前置验证代码 ...

    // 构建订单提交参数
    const orderParams = {
      symbol,
      orderType: OrderType.LO, // 限价单
      side: intent.side === 'BUY' ? OrderSide.Buy : OrderSide.Sell,
      submittedQuantity: intent.quantity,  // ← 问题点：可能是负数或非标准格式
      timeInForce: TimeInForceType.Day,
      submittedPrice: intent.price,
      outsideRth: OutsideRTH.AnyTime,
      remark: `策略${strategyId}`,
    };

    // 提交订单
    const result = await tradeClient.submitOrder(orderParams);
    // ...
  } catch (error: any) {
    logger.error(`策略 ${strategyId} 提交订单失败 (${symbol}):`, error);  // ← 第778行
    // ...
  }
}
```

#### 问题分析
- **根本原因**:
  1. 卖空订单的 `quantity` 是负数（如 -100）
  2. LongPort SDK要求 `submittedQuantity` 必须是正整数或正小数
  3. 没有对卖空订单的数量进行绝对值转换

- **影响标的**:
  - BMNR.US: 331次
  - HOOD.US: 296次
  - ACHR.US: 292次
  - PLTR.US: 206次
  - SHOP.US: 201次
  - ROKU.US: 182次
  - RBLX.US: 123次

#### 修复方案

##### 方案1: 修正订单数量处理（推荐）

在 `basic-execution.service.ts` 中修改：

```typescript
// 修改前：
const orderParams = {
  symbol,
  orderType: OrderType.LO,
  side: intent.side === 'BUY' ? OrderSide.Buy : OrderSide.Sell,
  submittedQuantity: intent.quantity,  // ← 可能是负数
  timeInForce: TimeInForceType.Day,
  submittedPrice: intent.price,
  outsideRth: OutsideRTH.AnyTime,
  remark: `策略${strategyId}`,
};

// 修改后：
const orderParams = {
  symbol,
  orderType: OrderType.LO,
  side: intent.side === 'BUY' ? OrderSide.Buy : OrderSide.Sell,
  submittedQuantity: Math.abs(intent.quantity).toString(),  // ← 转为正数并转字符串
  timeInForce: TimeInForceType.Day,
  submittedPrice: new Decimal(intent.price.toString()),  // ← 使用Decimal类型
  outsideRth: OutsideRTH.AnyTime,
  remark: `策略${strategyId}`,
};
```

##### 方案2: 在策略层面规范化数量

在 `strategy-scheduler.service.ts` 中，生成TradingIntent时确保数量为正：

```typescript
// 卖空意图
const shortIntent: TradingIntent = {
  action: 'SHORT',
  side: 'SELL',
  symbol: symbol,
  quantity: Math.abs(calculatedQuantity),  // ← 确保是正数
  price: shortPrice,
  reason: shortReason,
  timestamp: new Date(),
};
```

#### 建议
**同时实施方案1和方案2**，在多个层面确保数据格式正确。

---

### 4. 订单价格更新失败 - Decimal类型错误 (1,040次)

#### 错误信息
```
策略 5 标的 AMD.US 订单价格更新失败 (1200364832893718528):
Unwrap value [longport_nodejs::decimal::Decimal] from class failed on ReplaceOrderOptions.quantity
```

#### 代码位置
**文件**: `api/src/services/strategy-scheduler.service.ts`
**行号**: 853

#### 代码片段
```typescript:api/src/services/strategy-scheduler.service.ts
// 第800-860行
private async trackUnfilledOrders(strategyId: number): Promise<void> {
  try {
    // ... 查询未成交订单 ...

    for (const order of pendingOrders) {
      try {
        // 获取最新价格
        const latestPrice = await this.getLatestPrice(order.symbol);

        if (latestPrice) {
          // 更新订单价格
          await tradeClient.replaceOrder({
            orderId: order.order_id,
            quantity: order.quantity,  // ← 问题点：可能不是Decimal类型
            price: latestPrice,        // ← 问题点：可能不是Decimal类型
          });

          logger.log(`订单 ${order.order_id} 价格已更新: ${order.price} → ${latestPrice}`);
        }
      } catch (error: any) {
        const errorMessage = error.message || String(error);

        if (errorCode === '602012' || errorMessage.includes('602012') || errorMessage.includes('Order amendment is not supported')) {
          continue;
        }

        logger.warn(`策略 ${strategyId} 标的 ${order.symbol} 订单价格更新失败 (${order.order_id}): ${errorMessage}`);  // ← 第853行
      }
    }
  } catch (error: any) {
    logger.error(`追踪未成交订单失败 (策略 ${strategyId}):`, error);
  }
}
```

#### 问题分析
- **根本原因**:
  1. `replaceOrder` API要求 `quantity` 和 `price` 必须是 `Decimal` 类型
  2. 从数据库读取的值是 `number` 或 `string` 类型
  3. 没有进行类型转换

- **影响订单**:
  - AMD.US: 770次
  - RBLX.US: 246次
  - 其他多个标的

#### 修复方案

##### 方案1: 正确使用Decimal类型（推荐）

```typescript
import { Decimal } from '../config/longport';

// 修改前：
await tradeClient.replaceOrder({
  orderId: order.order_id,
  quantity: order.quantity,
  price: latestPrice,
});

// 修改后：
await tradeClient.replaceOrder({
  orderId: order.order_id,
  quantity: new Decimal(order.quantity.toString()),  // ← 转为Decimal
  price: new Decimal(latestPrice.toString()),        // ← 转为Decimal
});
```

##### 方案2: 创建类型转换辅助函数

在 `api/src/utils/decimal-helper.ts` 中：

```typescript
import { Decimal } from '../config/longport';

/**
 * 安全地转换为Decimal类型
 */
export function toDecimal(value: number | string | Decimal): Decimal {
  if (value instanceof Decimal) {
    return value;
  }
  return new Decimal(value.toString());
}

/**
 * 转换订单参数为正确的类型
 */
export function normalizeOrderParams(params: {
  quantity?: number | string | Decimal;
  price?: number | string | Decimal;
  [key: string]: any;
}) {
  const normalized: any = { ...params };

  if (params.quantity !== undefined) {
    normalized.quantity = toDecimal(params.quantity);
  }

  if (params.price !== undefined) {
    normalized.price = toDecimal(params.price);
  }

  return normalized;
}
```

使用：

```typescript
import { normalizeOrderParams } from '../utils/decimal-helper';

await tradeClient.replaceOrder(normalizeOrderParams({
  orderId: order.order_id,
  quantity: order.quantity,
  price: latestPrice,
}));
```

#### 建议
**推荐方案2**，统一处理所有Decimal类型转换，避免遗漏。

---

## ⚠️ 中优先级警告

### 5. 订单-信号关联失败 (1,867次)

#### 警告信息
```
未找到订单 1200364596456599552 关联的信号
(strategy_id=5, symbol=SHOP.US, side=SELL, signalType=SELL)
```

#### 代码位置
**文件**: `api/src/services/basic-execution.service.ts` 或订单关联逻辑
**预计位置**: 订单推送处理或信号匹配逻辑

#### 问题分析
- **根本原因**:
  1. 订单创建和信号创建的时间窗口不匹配
  2. 时间窗口过窄（当前1小时）
  3. 订单可能在信号创建之前就已存在（历史订单）

- **影响订单**:
  - TEM.US: 780次
  - SHOP.US: 774次
  - RBLX.US: 309次

#### 修复方案

需要查找订单-信号关联的具体代码位置，可能在：
1. `api/src/services/trade-push.service.ts`
2. `api/src/services/signal-order-matching.service.ts`
3. `strategy-scheduler.service.ts` 中的订单监控逻辑

##### 临时方案: 扩大时间窗口

```typescript
// 从1小时扩大到2小时
const timeWindow = {
  start: new Date(signal.created_at.getTime() - 2 * 60 * 60 * 1000),
  end: new Date(signal.created_at.getTime() + 2 * 60 * 60 * 1000)
};
```

##### 长期方案: 改进匹配算法

1. 增加订单ID和信号ID的直接关联字段
2. 在创建订单时立即关联信号ID
3. 使用事务确保订单和信号原子性创建

---

### 6. 持仓状态但context为空 (390次)

#### 警告信息
```
策略 5 标的 CRCL.US: 持仓状态但context为空
策略 5 标的 CRCL.US: 卖空持仓状态但context为空
```

#### 代码位置
**文件**: `api/src/services/strategy-scheduler.service.ts`
**行号**: 1629, 1951

#### 代码片段
```typescript:api/src/services/strategy-scheduler.service.ts
// 第1624-1632行
try {
  const contextData = instanceResult.rows[0].context;
  if (!contextData) {
    logger.warn(`策略 ${strategyId} 标的 ${symbol}: 持仓状态但context为空`);  // ← 第1629行
    return { actionTaken: false };
  } else {
    context = typeof contextData === 'string'
      ? JSON.parse(contextData)
      : contextData;
  }
}

// 第1946-1956行（卖空持仓）
let context: any = {};
try {
  const contextData = instanceResult.rows[0].context;
  if (!contextData) {
    logger.warn(`策略 ${strategyId} 标的 ${symbol}: 卖空持仓状态但context为空`);  // ← 第1951行
    return { actionTaken: false };
  } else {
    context = typeof contextData === 'string'
      ? JSON.parse(contextData)
      : contextData;
  }
}
```

#### 问题分析
- **根本原因**:
  1. 数据库中的 `strategy_instances` 表的 `context` 字段为NULL
  2. 可能是历史数据迁移不完整
  3. 可能是之前的代码没有正确保存context

- **影响标的**: CRCL.US (390次，正好对应390次策略执行)

#### 修复方案

##### 方案1: 修复历史数据

```sql
-- 查询context为空的持仓记录
SELECT id, strategy_id, symbol, status
FROM strategy_instances
WHERE status IN ('HOLDING', 'SHORT_HOLDING')
  AND (context IS NULL OR context = '{}');

-- 如果无法恢复context，将状态重置为IDLE
UPDATE strategy_instances
SET status = 'IDLE',
    context = NULL,
    updated_at = NOW()
WHERE status IN ('HOLDING', 'SHORT_HOLDING')
  AND (context IS NULL OR context = '{}');
```

##### 方案2: 代码中尝试恢复context

```typescript
if (!contextData) {
  // 尝试从订单历史恢复context
  const orderHistory = await pool.query(
    `SELECT * FROM orders
     WHERE strategy_id = $1 AND symbol = $2
     ORDER BY created_at DESC LIMIT 1`,
    [strategyId, symbol]
  );

  if (orderHistory.rows.length > 0) {
    const order = orderHistory.rows[0];
    context = {
      entryPrice: order.price,
      entryTime: order.created_at,
      quantity: order.quantity,
      orderId: order.id,
    };

    // 更新数据库
    await pool.query(
      `UPDATE strategy_instances
       SET context = $1
       WHERE strategy_id = $2 AND symbol = $3`,
      [JSON.stringify(context), strategyId, symbol]
    );

    logger.log(`已从订单历史恢复 ${symbol} 的context`);
  } else {
    logger.warn(`策略 ${strategyId} 标的 ${symbol}: 持仓状态但context为空，且无订单历史`);
    // 重置状态
    await pool.query(
      `UPDATE strategy_instances
       SET status = 'IDLE', context = NULL
       WHERE strategy_id = $1 AND symbol = $2`,
      [strategyId, symbol]
    );
    return { actionTaken: false };
  }
}
```

#### 建议
**先执行方案1清理数据，再实施方案2作为保护机制**。

---

### 7. 卖空验证失败 - API限流 (467次)

#### 警告信息
```
[策略执行] 策略 5 标的 TSLA.US: 卖空验证失败 -
Failed to check short selling permission: openapi error: code=429002
```

#### 代码位置
**文件**: `api/src/services/short-position-validation.service.ts`
**涉及的API调用**:
- `tradeClient.marginRatio()`
- 卖空权限检查相关API

#### 问题分析
- **根本原因**: 与问题2相同，API限流
- **影响标的**:
  - TSLA.US: 146次
  - ROKU.US: 118次
  - PLTR.US: 118次
  - SHOP.US: 85次

#### 修复方案
参考问题2的修复方案，在 `short-position-validation.service.ts` 中也应用速率限制器。

---

## 📋 修复优先级建议

### 立即修复（本周内完成）

1. ✅ **创建数据库表** (问题1)
   - 影响: 780次错误
   - 难度: 低
   - 时间: 10分钟

2. ✅ **修复Decimal类型转换** (问题3, 4)
   - 影响: 2,348次错误
   - 难度: 中
   - 时间: 2小时

### 短期优化（两周内完成）

3. ⏳ **实现API速率限制** (问题2, 7)
   - 影响: 1,555次错误/警告
   - 难度: 中
   - 时间: 4小时

4. ⏳ **清理持仓context数据** (问题6)
   - 影响: 390次警告
   - 难度: 低
   - 时间: 1小时

### 长期改进（一个月内完成）

5. ⏳ **优化订单-信号关联** (问题5)
   - 影响: 1,867次警告
   - 难度: 高
   - 时间: 1天

6. ⏳ **添加监控和告警**
   - 实时监控错误率
   - 自动告警机制
   - 性能指标追踪

---

## 🔧 通用修复建议

### 1. 代码质量提升

#### 类型安全
```typescript
// 在所有API调用处添加类型检查
import { Decimal } from '../config/longport';

function ensureDecimal(value: any): Decimal {
  if (value instanceof Decimal) return value;
  if (typeof value === 'number' || typeof value === 'string') {
    return new Decimal(value.toString());
  }
  throw new Error(`无法转换为Decimal: ${value}`);
}
```

#### 错误处理
```typescript
// 统一的错误处理包装器
async function safeExecute<T>(
  fn: () => Promise<T>,
  context: string
): Promise<T | null> {
  try {
    return await fn();
  } catch (error: any) {
    logger.error(`${context} 执行失败:`, error);

    // 根据错误类型决定是否重试
    if (error.message?.includes('429002')) {
      // API限流，等待后重试
      await new Promise(r => setTimeout(r, 2000));
      return await fn();
    }

    return null;
  }
}
```

### 2. 数据库迁移管理

**已创建迁移脚本**: `api/migrations/011_create_validation_failure_logs.sql`

该脚本已按照项目迁移规范创建，包含：
- 完整的表结构定义
- 外键约束
- 性能优化索引
- 中英文注释
- 自动验证逻辑

执行方式：

```bash
# 方式1: 使用psql命令行工具
psql -U postgres -d trading_db -f api/migrations/011_create_validation_failure_logs.sql

# 方式2: 使用pgAdmin或其他数据库管理工具
# 直接打开文件并执行SQL脚本
```

如果需要回滚（删除表）：

```sql
-- 谨慎操作：删除validation_failure_logs表
DROP TABLE IF EXISTS validation_failure_logs CASCADE;
```

### 3. 日志增强

在关键位置添加更详细的日志：

```typescript
// 在订单提交前记录详细参数
logger.debug(`准备提交订单:`, {
  strategyId,
  symbol,
  side: intent.side,
  quantity: intent.quantity,
  quantityType: typeof intent.quantity,
  price: intent.price,
  priceType: typeof intent.price,
});

// 在API调用后记录响应
logger.debug(`API调用成功:`, {
  api: 'submitOrder',
  symbol,
  orderId: result.orderId,
  responseTime: Date.now() - startTime,
});
```

### 4. 单元测试

为关键函数添加测试：

```typescript
// api/src/__tests__/decimal-conversion.test.ts
import { toDecimal } from '../utils/decimal-helper';
import { Decimal } from '../config/longport';

describe('Decimal转换测试', () => {
  test('应该正确转换number为Decimal', () => {
    const result = toDecimal(100);
    expect(result).toBeInstanceOf(Decimal);
    expect(result.toString()).toBe('100');
  });

  test('应该正确转换string为Decimal', () => {
    const result = toDecimal('99.99');
    expect(result).toBeInstanceOf(Decimal);
    expect(result.toString()).toBe('99.99');
  });

  test('应该正确处理负数', () => {
    const result = toDecimal(-50);
    expect(result.toString()).toBe('-50');
  });

  test('已经是Decimal则直接返回', () => {
    const original = new Decimal('123.45');
    const result = toDecimal(original);
    expect(result).toBe(original);
  });
});
```

---

## 📊 预期改进效果

### 修复前
- 错误率: 15.2% (3,737/24,630)
- 警告率: 18.2% (4,479/24,630)
- 主要问题: 数据库错误、API限流、类型转换

### 修复后预期
- 错误率: <5% (目标 <1,232)
- 警告率: <10% (目标 <2,463)
- 主要改进:
  - 数据库错误: 780 → 0 ✅
  - Decimal错误: 2,348 → 0 ✅
  - API限流: 1,088 → <100 📉
  - 订单关联: 1,867 → <500 📉

---

## 📝 执行清单

### 第一阶段：数据库修复
- [x] 执行迁移脚本 `api/migrations/011_create_validation_failure_logs.sql`（✅ 已执行）
  ```bash
  psql -U postgres -d trading_db -f api/migrations/011_create_validation_failure_logs.sql
  ```
- [ ] 验证表创建成功
  ```sql
  SELECT table_name FROM information_schema.tables WHERE table_name = 'validation_failure_logs';
  \d validation_failure_logs
  ```
- [ ] 测试日志记录功能
  ```sql
  INSERT INTO validation_failure_logs (strategy_id, symbol, failure_type, reason, timestamp)
  VALUES (1, 'TEST.US', 'TEST', 'Test record', NOW());
  DELETE FROM validation_failure_logs WHERE failure_type = 'TEST';
  ```

### 第二阶段：代码修复
- [x] 修复 `basic-execution.service.ts`：卖空订单 `submittedQuantity` 统一转正数 + 使用 `Decimal`
- [x] 修复 `strategy-scheduler.service.ts`：`replaceOrder.quantity` 使用 `Decimal`，卖空验证失败写入 `validation_failure_logs`
- [x] 修复 `api/src/routes/orders.ts`：`replaceOrder.quantity` 使用 `Decimal`（避免 unwrap 报错）
- [x] 修复 `strategy-scheduler.service.ts`：持仓/卖空持仓 `context` 为空时尝试从订单历史恢复（无历史则重置为 `IDLE`）
- [ ] 添加单元测试（建议覆盖：卖空下单数量、replaceOrder参数、context恢复）

### 第三阶段：速率限制
- [x] 创建 `api/src/utils/longport-rate-limiter.ts`（轻量队列限流 + 429002指数退避重试）
- [x] 在关键LongPort调用处应用（`submitOrder` / `replaceOrder` / `accountBalance`）
- [ ] 评估并扩展到其它高频调用点（如 quote/staticInfo/todayOrders 等）
- [ ] 测试限流效果（观察 429002 日志是否显著下降）

### 第四阶段：数据清理
- [ ] 清理持仓context为空的记录
- [ ] 添加context恢复逻辑
- [ ] 验证数据一致性

### 第五阶段：测试验证
- [ ] 运行单元测试
- [ ] 运行集成测试
- [ ] 监控线上日志
- [ ] 验证错误率下降

---

## 🔗 相关文档

- [日志分析看板](./logs-analysis-dashboard.html)
- [日志分析使用说明](./日志分析看板使用说明.md)
- [工具使用指南](./scripts/LOG_ANALYSIS_README.md)
- [卖空功能代码审查](./docs/review/251225-卖空功能代码审查报告.md)

---

**文档版本**: 1.0
**生成时间**: 2026-01-27
**基于日志**: logs-2026-01-27.json (24,630条)
**维护者**: Trading System Team
