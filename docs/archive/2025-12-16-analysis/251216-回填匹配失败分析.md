# 回填匹配失败分析

## 📋 问题描述

**清理后重新运行回填脚本**：
- PENDING信号数量：从40,193减少到29,858（减少25.8%）
- **匹配结果**：仍然是0个匹配成功
- 未找到信号的订单数：45
- 未找到订单的信号数：29,858

**结论**：问题不是数据量太大，而是**时间窗口匹配逻辑的问题**。

---

## 🔍 根本原因分析

### 问题1：时间窗口太小 ⚠️ **高概率**

**当前设置**：默认±5分钟

**问题**：
- 信号和订单的创建时间可能差距较大（超过5分钟）
- 策略生成信号后，可能需要更长时间才执行订单
- 或者订单是手动提交的，时间差距更大

**验证方法**：
```sql
-- 检查订单和信号的时间差分布
SELECT 
  eo.order_id,
  eo.created_at as order_time,
  ss.id as signal_id,
  ss.created_at as signal_time,
  ABS(EXTRACT(EPOCH FROM (eo.created_at - ss.created_at))) / 60 as time_diff_minutes
FROM execution_orders eo
CROSS JOIN strategy_signals ss
WHERE eo.strategy_id = ss.strategy_id
  AND eo.symbol = ss.symbol
  AND (
    (eo.side = 'BUY' AND ss.signal_type = 'BUY')
    OR (eo.side = 'SELL' AND ss.signal_type = 'SELL')
  )
  AND eo.signal_id IS NULL
  AND ss.status = 'PENDING'
ORDER BY time_diff_minutes
LIMIT 100;
```

### 问题2：匹配条件太严格 ⚠️ **中概率**

**当前匹配条件**：
- strategy_id 必须相同
- symbol 必须相同
- side 必须匹配（BUY/SELL）
- 时间窗口±5分钟
- 信号状态必须是PENDING

**问题**：
- side的格式可能不一致（BUY vs Buy）
- symbol格式可能不一致（.US vs .us）
- 信号状态可能已经改变

**验证方法**：
```sql
-- 检查side格式分布
SELECT DISTINCT side FROM execution_orders WHERE signal_id IS NULL;
SELECT DISTINCT signal_type FROM strategy_signals WHERE status = 'PENDING';

-- 检查symbol格式
SELECT DISTINCT symbol FROM execution_orders WHERE signal_id IS NULL LIMIT 10;
SELECT DISTINCT symbol FROM strategy_signals WHERE status = 'PENDING' LIMIT 10;
```

### 问题3：信号和订单不匹配 ⚠️ **中概率**

**问题**：
- 订单可能是手动提交的，没有对应的信号
- 或者信号生成后，订单执行失败，没有关联
- 或者订单是系统自动生成的（如止损/止盈），没有对应的信号

**验证方法**：
```sql
-- 检查订单的创建时间分布
SELECT 
  DATE(created_at) as date,
  COUNT(*) as count,
  MIN(created_at) as earliest,
  MAX(created_at) as latest
FROM execution_orders
WHERE signal_id IS NULL
GROUP BY DATE(created_at)
ORDER BY date DESC
LIMIT 30;

-- 检查信号的创建时间分布
SELECT 
  DATE(created_at) as date,
  COUNT(*) as count,
  MIN(created_at) as earliest,
  MAX(created_at) as latest
FROM strategy_signals
WHERE status = 'PENDING'
GROUP BY DATE(created_at)
ORDER BY date DESC
LIMIT 30;
```

---

## 💡 解决方案

### 方案1：扩大时间窗口 ⭐ **推荐先尝试**

**实施步骤**：

1. **使用30分钟时间窗口测试**：
   ```bash
   npm run backfill-signals:dry-run -- --time-window-minutes=30
   ```

2. **如果仍然匹配失败，尝试60分钟**：
   ```bash
   npm run backfill-signals:dry-run -- --time-window-minutes=60
   ```

3. **如果仍然匹配失败，尝试120分钟**：
   ```bash
   npm run backfill-signals:dry-run -- --time-window-minutes=120
   ```

**注意事项**：
- ⚠️ 时间窗口越大，误匹配的风险越高
- ⚠️ 建议先运行dry-run模式查看结果
- ⚠️ 逐步扩大时间窗口，找到最佳平衡点

### 方案2：分析数据时间差分布 ⭐ **推荐先执行**

**执行SQL查询，了解数据情况**：

```sql
-- 查询1：检查订单和信号的时间差分布
SELECT 
  CASE 
    WHEN time_diff <= 5 THEN '0-5分钟'
    WHEN time_diff <= 15 THEN '5-15分钟'
    WHEN time_diff <= 30 THEN '15-30分钟'
    WHEN time_diff <= 60 THEN '30-60分钟'
    WHEN time_diff <= 120 THEN '60-120分钟'
    ELSE '120分钟以上'
  END as time_range,
  COUNT(*) as count
FROM (
  SELECT 
    eo.order_id,
    ss.id as signal_id,
    ABS(EXTRACT(EPOCH FROM (eo.created_at - ss.created_at))) / 60 as time_diff
  FROM execution_orders eo
  CROSS JOIN strategy_signals ss
  WHERE eo.strategy_id = ss.strategy_id
    AND eo.symbol = ss.symbol
    AND (
      (eo.side = 'BUY' AND ss.signal_type = 'BUY')
      OR (eo.side = 'SELL' AND ss.signal_type = 'SELL')
    )
    AND eo.signal_id IS NULL
    AND ss.status = 'PENDING'
) t
GROUP BY time_range
ORDER BY time_range;
```

**根据查询结果决定**：
- 如果大部分时间差在5-30分钟：使用30分钟时间窗口
- 如果大部分时间差在30-60分钟：使用60分钟时间窗口
- 如果大部分时间差在60分钟以上：考虑其他方案

### 方案3：优化匹配逻辑 ⭐ **长期优化**

**改进建议**：

1. **放宽side匹配**：
   - 支持BUY/Buy/buy等多种格式
   - 支持SELL/Sell/sell等多种格式

2. **放宽symbol匹配**：
   - 支持大小写不敏感匹配
   - 支持.US/.us等多种格式

3. **优先级匹配**：
   - 优先匹配订单创建时间之前的信号（最近的）
   - 降级匹配订单创建时间之后的信号（最近的）

4. **添加手动匹配功能**：
   - 对于无法自动匹配的订单，提供手动关联界面

### 方案4：清理无法匹配的信号 ⭐ **可选**

**如果确认大部分PENDING信号都无法匹配**：

```bash
# 清理所有非持仓标的的PENDING信号（无论时间）
# 注意：此操作较激进，建议先备份数据库
```

**或者**：
```sql
-- 将超过30天未匹配的PENDING信号标记为IGNORED
UPDATE strategy_signals
SET status = 'IGNORED'
WHERE status = 'PENDING'
  AND created_at < NOW() - INTERVAL '30 days'
  AND symbol NOT IN (
    SELECT DISTINCT symbol 
    FROM strategy_instances 
    WHERE current_state = 'HOLDING'
  );
```

---

## 🚀 推荐执行步骤

### 步骤1：分析数据时间差分布 ⭐ **必须先执行**

运行SQL查询，了解订单和信号的时间差分布：

```sql
-- 检查时间差分布
SELECT 
  CASE 
    WHEN time_diff <= 5 THEN '0-5分钟'
    WHEN time_diff <= 15 THEN '5-15分钟'
    WHEN time_diff <= 30 THEN '15-30分钟'
    WHEN time_diff <= 60 THEN '30-60分钟'
    WHEN time_diff <= 120 THEN '60-120分钟'
    ELSE '120分钟以上'
  END as time_range,
  COUNT(*) as count
FROM (
  SELECT 
    ABS(EXTRACT(EPOCH FROM (eo.created_at - ss.created_at))) / 60 as time_diff
  FROM execution_orders eo
  CROSS JOIN strategy_signals ss
  WHERE eo.strategy_id = ss.strategy_id
    AND eo.symbol = ss.symbol
    AND (
      (eo.side = 'BUY' AND ss.signal_type = 'BUY')
      OR (eo.side = 'SELL' AND ss.signal_type = 'SELL')
    )
    AND eo.signal_id IS NULL
    AND ss.status = 'PENDING'
) t
GROUP BY time_range
ORDER BY time_range;
```

### 步骤2：根据分析结果扩大时间窗口

**如果时间差主要在5-30分钟**：
```bash
npm run backfill-signals:dry-run -- --time-window-minutes=30
```

**如果时间差主要在30-60分钟**：
```bash
npm run backfill-signals:dry-run -- --time-window-minutes=60
```

**如果时间差主要在60分钟以上**：
- 考虑使用更大的时间窗口（120分钟或更长）
- 或者考虑这些信号和订单可能不匹配

### 步骤3：验证匹配结果

如果匹配成功，查看匹配的订单和信号：

```sql
-- 检查匹配成功的订单
SELECT 
  eo.order_id,
  eo.symbol,
  eo.side,
  eo.created_at as order_time,
  ss.id as signal_id,
  ss.created_at as signal_time,
  ABS(EXTRACT(EPOCH FROM (eo.created_at - ss.created_at))) / 60 as time_diff_minutes
FROM execution_orders eo
JOIN strategy_signals ss ON eo.signal_id = ss.id
WHERE eo.signal_id IS NOT NULL
ORDER BY eo.created_at DESC
LIMIT 20;
```

---

## ⚠️ 重要提醒

1. **数据备份**：执行任何数据修改前，确保数据库有备份
2. **逐步测试**：先运行dry-run模式，确认结果后再实际执行
3. **时间窗口平衡**：时间窗口越大，误匹配的风险越高
4. **手动验证**：匹配成功后，建议手动验证几个订单和信号的关联是否正确

---

## 📝 总结

**当前情况**：
- ❌ 0个订单和信号匹配成功
- ⚠️ 29,858个PENDING信号未关联
- ⚠️ 45个订单未关联信号

**可能原因**：
1. 时间窗口太小（默认5分钟）
2. 信号和订单的创建时间差距较大
3. 匹配条件太严格

**建议行动**：
1. ✅ **先分析数据**：运行SQL查询，了解时间差分布
2. ✅ **扩大时间窗口**：根据分析结果，使用30-120分钟的时间窗口
3. ✅ **验证匹配结果**：匹配成功后，验证关联是否正确
4. ⏳ **长期优化**：优化匹配逻辑，支持更灵活的匹配条件

---

**分析完成时间**：2025-12-16  
**下次更新计划**：扩大时间窗口测试后更新





