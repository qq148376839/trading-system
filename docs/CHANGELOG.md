...
**背景**: 基于 2026-02-06 实盘数据分析——部署后 +$48（几乎持平），9 笔超短持仓亏 -$200，36 笔无效取消订单。

**实现内容**:

#### 1. 动态止损冷静期
- 持仓 0-3 分钟：跳过常规止损，仅安全阀（50%）生效
- 持仓 3-10 分钟：止损线放宽 1.5 倍
- 持仓 10+ 分钟：标准止损（原逻辑不变）
- 安全阀（50%绝对止损）全时段保持不变

#### 2. 时间窗口收紧
- `noNewEntryBeforeCloseMinutes` 从 60 改为 120
- 14:00 ET 后不再开新仓（原 15:00 ET），消除无效取消订单

#### 3. 取消订单退避
- 订单取消/拒绝时记录 `cancelCount` 和 `lastCancelTime` 到 context
- IDLE 路径添加指数退避：5min → 10min → 20min → 30min(上限)
- 成功开仓时 context 自动覆盖，退避元数据自然清理

**变更文件**:
- `api/src/services/option-dynamic-exit.service.ts` — 止损冷静期逻辑
- `api/src/services/strategies/option-intraday-strategy.ts` — 时间窗口默认值
- `api/src/services/strategy-scheduler.service.ts` — 取消退避存储与检查

**相关文档**:
- [期权策略优化实施文档](docs/features/260203-期权策略优化实施文档.md) — 已更新迭代记录
...
**优化**: 基于实盘运行数据，对期权推荐算法、订单执行、资金管理进行迭代优化

**实现内容**:

#### 1. 推荐算法权重重构
- 综合得分权重从 大盘40%/分时40%/时间窗口20% 改为 大盘50%/分时50%/时间窗口×0.15修正
- 新增 SPX 当日涨跌幅信号（大盘评分中权重35%，最重要的信号）
- 趋势分析引入 MA5/MA10/MA20 三均线排列，放大系数从10倍提升到30倍
- 动量计算从逐bar平均改为首尾累计变化率×方向一致性，避免震荡中正负抵消
- 每次评分输出明细日志（大盘评分明细、分时评分明细），提升可观测性

#### 2. 期权订单全面市价单
- 所有期权订单（买入+卖出）均使用市价单，实现快进快出
- Fallback 限价单区分方向：买入用 `当前价×1.05`，卖出用 `当前价×0.1`

#### 3. 资金管理支持期权合约
- 期权合约通过 `strategy_instances` 表查询已用资金（因 auto_trades 存的是 underlying symbol）
- 引入 `isLikelyOptionSymbol()` 自动区分期权/股票查询路径

#### 4. 策略执行改进
- 入场决策从使用 `marketScore` 改为 `finalScore`（综合得分），激进模式阈值降至10
- 期权卖出数量以券商实际持仓为准（DB记录不一致时打 WARN 日志）
- 占用槽位计算扩展至 HOLDING + OPENING + CLOSING 状态

#### 5. 迁移脚本合并
- 将 010~013 及 `create_order_prevention_metrics_table.sql` 合并入 `000_init_schema.sql`
- 新增日志节流/摘要配置项（log_throttle_enabled、log_digest_enabled 等）

#### 6. 修复
- 修复 `log-worker.service.ts` 重复 `infraLogger` 导入

**变更文件**:
- `api/src/services/option-recommendation.service.ts` — 算法权重重构
- `api/src/services/basic-execution.service.ts` — 市价单全覆盖
- `api/src/services/capital-manager.service.ts` — 期权资金查询
- `api/src/services/strategies/option-intraday-strategy.ts` — 综合得分+阈值调整
- `api/src/services/strategy-scheduler.service.ts` — 卖出数量校正+槽位计算
- `api/src/services/log-worker.service.ts` — 重复导入修复
- `api/migrations/000_init_schema.sql` — 迁移脚本合并

**相关文档**:
- [期权策略优化实施文档](docs/features/260203-期权策略优化实施文档.md) — 已更新算法详解和迭代记录
...
**优化**: 实现级别门控、节流机制、摘要聚合，DB写入量减少95%+

**实现内容**:
1. 级别门控：DEBUG 仅控制台，ERROR 必入库不节流，WARN/INFO 走节流
2. 节流机制：同一消息模板在窗口内（默认30秒）只写一条到库
3. 摘要聚合服务：高频指标每5分钟写一条聚合日志
4. 基础设施轻量 Logger（infra-logger），供底层模块使用避免循环依赖
...
**功能**: 期权策略决策日志写入独立数据库表，支持历史查询和分析

**相关文档**:
- [期权策略决策日志数据库持久化](docs/features/260205-期权策略决策日志数据库持久化.md)
...
**功能**: 创建期权专用推荐服务，替换股票推荐逻辑；市场数据获取添加重试机制

**相关文档**:
- [期权策略优化实施文档](docs/features/260203-期权策略优化实施文档.md)
- [Docker单容器部署架构改造文档](docs/features/260203-Docker单容器部署架构改造文档.md)
