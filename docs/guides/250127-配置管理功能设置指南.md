...
| `only_regular_hours` | 是否仅允许盘中交易时段（true=仅盘中，false=包含盘前盘后） | ❌ | true |
...
- **only_regular_hours**: 控制策略是否仅在盘中交易时段执行
  - `true`（默认）：仅允许盘中交易时段（9:30-16:00），排除盘前（4:00-9:30）和盘后（16:00-20:00）
  - `false`：允许所有交易时段，包括盘前和盘后
  - **注意**：此配置影响策略调度器的执行时间，建议根据实际交易需求设置
...
### 2025-12-24
- ✅ 添加 `only_regular_hours` 配置项，支持通过配置控制是否仅允许盘中交易时段
- ✅ 交易时段服务从配置中读取 `only_regular_hours` 设置，不再硬编码
