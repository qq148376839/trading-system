...
- **期权回测** tab：
  - 结果列表表格：策略、日期、标的、状态、交易数、胜率、总 PnL、平均 PnL%、盈利因子
  - "执行期权回测"弹窗：
    - 策略选择器（筛选 OPTION_INTRADAY_V1 类型策略）
    - 自动展示策略配置的标的（只读 tag）
    - 多日期选择器
    - 策略参数卡（可选覆盖）：入场阈值、风险偏好、合约数、每日最大交易、交易窗口
...
| DB 存储 | 复用 `backtest_results` 表，使用实际 `strategy_id` | 满足 FK 约束，关联策略配置 |
