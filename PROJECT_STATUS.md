...
**改动**: 修复3个bug — POSITION_EXIT_CLEANUP 移除 consecutiveLossPctSum 防止清理型路径破坏A1门控数据一致性；ATR缓存改为逐个补充缺失标的；A4注释修正。
...
**改动**: `getCurrentMarketScore()` 原使用简化版 intradayScore（仅 SPX 动量），与策略实际用的完整版不一致（60% 权重偏差）。重写为直接调用 `calculateOptionRecommendation('.SPX.US')` 复用完整计算路径。

**修改文件**: `option-recommendation.service.ts`

### 2026-04-06: 监控大屏时间戳实时读秒

**改动**: 时间戳改为 `Date.now()` 实时时钟每秒刷新。

**修改文件**: `monitor/page.tsx`
...
**修改文件**: `docs/analysis/` 下 3 份分析文档（纯文档变更）
...
### 2026-03-28: 交易分析模块数据准确性修复

**改动**: 4 项修复 — 前端 PnL 改用实际订单成交价（消除信号估算偏差，修复 1 笔盈亏方向翻转）；auto_trades 期权 PnL ×100 乘数；BUY 信号配对排除 FILTERED 状态；PnL% 语义修正。

**修改文件**: `frontend/app/quant/analysis/page.tsx`, `api/src/services/basic-execution.service.ts`
...
**问题**: 每日重置只扫描 non-IDLE 实例，IDLE 标的的 dailyRealizedPnL 从前几天累积到今天，导致熔断误触发（$2470 vs 实际 $48）。

**修复**: 主重置扫描所有实例 + 无时间戳但有残留 PnL 的兜底重置。

**修改文件**: `api/src/services/strategy-scheduler.service.ts`

### 2026-03-19: 期权评分夏令时偏移修复

**问题**: 时间窗口调整和时间衰减因子硬编码 EST (UTC-5)，夏令时期间偏移 1 小时，10:30-11:30 ET 评分虚高 ~4 分。

**修复**: 提取 `getETMinutes()` 统一使用 `America/New_York` 时区，3 个时间函数复用。

**修改文件**: `api/src/services/option-recommendation.service.ts`

### 2026-03-17: 期权资金预检拦截

**问题**: MU 期权1张合约成本超过单标的预算，`generateSignal()` 强制最低1张合约，每周期生成无法执行的 PENDING 信号，累积 172 条。

**修复**: 在 `generateSignal()` 的 MAX_PREMIUM 仓位计算中增加预检 — 1张合约都超预算时直接拒绝信号，并缓存结果5分钟避免重复评估。被拦截标的自然排除在竞价外，其他标的正常竞争。

**修改文件**: `api/src/services/strategies/option-intraday-strategy.ts`

### 2026-03-17: 退出后同组标的冷却联动

**问题**: TSLA 期权退出后 30 秒内同策略对不同行权价重新开仓，造成无效换仓和摩擦损耗。

**修复**: 新增 `recentExits` 机制 — 退出后冷却期内同组标的不参与竞价，9 个退出路径统一走 `recordSymbolExit()`，同时修复 MIT 回调遗漏的 activeEntries.delete。

**修改文件**: `api/src/services/strategy-scheduler.service.ts`

### 2026-03-17: 修复持仓同步问题（跨策略幽灵持仓 + 看门狗死循环）

**问题**: 策略10卖出期权后，策略12/13仍显示HOLDING，0DTE看门狗反复平仓产生被拒订单死循环。

**修复**:
1. **看门狗券商预检**: 平仓前查询券商实际持仓，无仓位直接清理DB
2. **submitted+rejected 误判**: 被拒订单不再误判为成功
3. **重试兜底**: 3次失败后再查券商，无仓位则清理
4. **跨策略同步（根因）**: 卖出成交后同步清理同合约的其他策略幽灵HOLDING
...
8. **反向盈亏修正 (3/15)**: 按实际 entry/exit 时间找最近反向K线重算反向价格及盈亏；反向分析表日期格式化
...
1. **编辑模式**: 策略详情页「编辑分组」按钮，进入可视化编辑面板
2. **自动合并**: 设定目标分组数，按成员数保留大组、剩余合并
3. **标的移动**: 点击 Tag × 弹出组选择器，在组间拖移标的
4. **manualOverride 标记**: 保存后引擎直接使用手动分组，标的变动时跳过自动重算
